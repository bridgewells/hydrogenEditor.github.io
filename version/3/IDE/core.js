...
let sessionManager = {
	...
	changeSession:function(path){
		...
		currentlyLoadedPath = path;
		if (getFileExtension(path).toLowerCase() == "html"){
			lastHTMLpath = path;
			microManager.init('file://'+path);
		}
	},
...
function saveEditorContent(){
	...
	if (liveEditorOpened){
		...
		$(".liveEditor").contents().find("html").find("style[data-href]").replaceWith(function(){
			return '<link rel="stylesheet" href="' + $(this).data('href') + '"/>';
		});
...
function createDeleteItemPanel(withContent){
	...
	$('#delText').text('Are you sure you want to delete '+filename+'? \t(This cannot be undone!)');
...
$(document.body).on('click','.dropdown-content>a',function(s){
	$('#pointer-hover-button').text(s.target.innerHTML)
});